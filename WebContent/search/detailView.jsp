...
<script type="text/javascript"> 
	function reservation(title){
		if('<%=s%>' == "admin77") alert("관리자는 예약 서비스를 이용하실 수 없습니다.");
		else if('<%=s%>' == "guest") {
			alert("로그인 후에 이용할 수 있습니다.");
		}else{
			opener.document.location.href='../book/reserve_proc.jsp?title='+title;
			window.close();
		}
	}
</script>
    ...
    <br>
    	<input type="button" value="예약하기" onclick="reservation('<%=bdto.getTitle()%>')">
